...
void MyWindow::on_pushButtonLogin_clicked()
{
  ...
  if(nouvelUtilisateur==1){
    ...
    if(i>0){
      printf("%d",estPresent(nom));
      if(verifieMotDePasse(i,motDePasse)==1){
...
void MyWindow::on_pushButtonAfficheFichier_clicked()
{
  ...
  i=listeUtilisateurs(vecteur);
  printf("%d",i);
  for(j=0;j<i;j++){
    ajouteTupleTableUtilisateurs(vecteur[j].nom,vecteur[j].hash);
  }
